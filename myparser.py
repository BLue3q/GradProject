...
def p_stmt(p):
    ...
    if len(p) == 7 and p[1] == 'class':  # Class declaration
            ...
            if member.get('type') == 'member_variable':
                ...
                for stmt in member['body']:
                    if stmt.get('type') == 'declaration':
                        ...
                        stmt['scope'] = func_scope
                    elif stmt.get('type') == 'class_pointer_declaration':  # Added class pointer declaration handling
                        stmt['scope'] = func_scope
                pop_scope()
        p[0] = {
        ...
        for stmt in p[7]:
            if stmt.get('type') == 'declaration':
                ...
                stmt['scope'] = func_scope
                if stmt.get('value') and isinstance(stmt['value'], dict) and stmt['value'].get('type') == 'variable':
                    stmt['value']['scope'] = func_scope
        ...
        for stmt in p[6]:
            if stmt.get('type') == 'declaration':
                ...
                stmt['scope'] = 'function:main'
                if stmt.get('value') and isinstance(stmt['value'], dict) and stmt['value'].get('type') == 'variable':
                    stmt['value']['scope'] = 'function:main'
            elif stmt.get('type') == 'member_assignment':
                stmt['scope'] = 'function:main'
                if stmt.get('value') and isinstance(stmt['value'], dict) and stmt['value'].get('type') == 'variable':
                    stmt['value']['scope'] = 'function:main'
    ...
    elif len(p) == 7:  # member access assignment (obj.member = value or ptr->member = value)
        current_scope = get_current_scope()
